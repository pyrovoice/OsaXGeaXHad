...
func onMouseMove(position, collider):
    ...
    if collider is Character:
        draw3D.line(ally.position + Vector3(0, 1, 0), collider.position + Vector3(0, 1, 0))
    elif collider is Cover:
        highlightPathTo(collider.position)
    else:
...
func onMouseClick(position, collider):
    ...
    if collider is Character:
        if selectedTarget && selectedTarget == collider:
            ...
            draw3D.line(ally.position+ Vector3(0, 1, 0), collider.position+ Vector3(0, 1, 0), Color.RED)
    elif collider is Cover:
        print("Undercover")
        collider._show()
        lastShownCover = collider
        tryMoveCharacterTo(collider.position)
    else:
